    ...
    <script>
        ...
        var DtSetter = function(tar, val) {
            var defines = {
                ...
                'ownerList': JSON.parse('<?= json_encode(isset($owners) ? $owners : []) ?>'),
                'packages': JSON.parse('<?= json_encode(isset($packages) ? $packages : []) ?>'),
                'expense_type': JSON.parse('<?= json_encode($this->Do->get('expense_type')) ?>'),

            }
            if (tar == 'servicePackage') {
                console.log('servicePackage', defines[tar])
            }
        ...
        (function() {
            ...
            const urls_source = JSON.parse('<?= json_encode([]) ?>');
            ...
            app.controller('ctrl', function($scope, $http, $location, $timeout, $q, $compile, $filter, $interval) {
                ...
                $scope.isExpiringSoon = function(dateString, periodControl) {
                    var currentDate = new Date();
                    var expirationDate = new Date(dateString);

                    // period_control değeri tarihe eklenir
                    expirationDate.setDate(expirationDate.getDate() + parseInt(periodControl));

                    var daysRemaining = Math.floor((expirationDate - currentDate) / (1000 * 60 * 60 * 24));

                    return daysRemaining <= 5; // Eğer 5 günden az kaldıysa true döner
                };

                $scope.rec = {
                    category: rec_origin.category,
                    content: rec_origin.content,
                    //inspect_rate: rec_origin.inspect_rate,
                }
                ...
                $scope.loopInUrls = function() {
                    ...
                    _doRequest('/admin/contents/save/-1', {
                        content_src: urls_source[inc]
                    }, 'post').then(function(res) {
