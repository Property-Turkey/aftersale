...
class UsersController extends AppController
{
    ...
    public function register() 
    {
        ...
        if ($this->request->is('post')) {
            ...
            //take email,password,mobile,fullname,address from userDT

            $email=$userDT['email'];
            $password=$userDT['password'];
            $fullname=$userDT['user_fullname'];
            $address=$userDT['address'];
            $mobile=$userDT['mobile'];
            
            //Check if email, mobile, password, fullname, address is empty
    if(empty($userDT['email']) || empty($userDT['password']) 
    || empty($userDT['mobile']) || empty($userDT['user_fullname']) || empty($userDT['address'])){
    if(@$_GET['ajax'] == 1){echo json_encode(["status"=>"invalid_credentials"]); die ;}
    $this->Flash->error(__('register-fail'));
}
            $userDT['user_role'] = $userDT['user_type'];
